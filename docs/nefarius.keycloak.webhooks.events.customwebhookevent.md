...
An application-defined event published through Keycloak's realm `/events` resource.

```csharp
public sealed class CustomWebhookEvent : WebhookBaseEvent
```

Inheritance [Object](https://learn.microsoft.com/dotnet/api/system.object) → [WebhookBaseEvent](./nefarius.keycloak.webhooks.events.webhookbaseevent.md) → [CustomWebhookEvent](./nefarius.keycloak.webhooks.events.customwebhookevent.md)
