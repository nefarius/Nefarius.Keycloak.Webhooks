...
A user has been deleted (`admin.USER-DELETE`).

```csharp
public sealed class AdminUserDeletedEvent : AdminWebhookEvent
```

Inheritance [Object](https://learn.microsoft.com/dotnet/api/system.object) → [WebhookBaseEvent](./nefarius.keycloak.webhooks.events.webhookbaseevent.md) → [AdminWebhookEvent](./nefarius.keycloak.webhooks.events.adminwebhookevent.md) → [AdminUserDeletedEvent](./nefarius.keycloak.webhooks.events.adminuserdeletedevent.md)<br>
...
### <a id="constructors-.ctor"/>**AdminUserDeletedEvent()**
