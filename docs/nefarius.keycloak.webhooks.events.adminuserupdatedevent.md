...
An administrator has updated a user (`admin.USER-UPDATE`).

```csharp
public sealed class AdminUserUpdatedEvent : AdminWebhookEvent
```

Inheritance [Object](https://learn.microsoft.com/dotnet/api/system.object) → [WebhookBaseEvent](./nefarius.keycloak.webhooks.events.webhookbaseevent.md) → [AdminWebhookEvent](./nefarius.keycloak.webhooks.events.adminwebhookevent.md) → [AdminUserUpdatedEvent](./nefarius.keycloak.webhooks.events.adminuserupdatedevent.md)<br>
...
Keycloak internal ID of the updated user.
...
### <a id="constructors-.ctor"/>**AdminUserUpdatedEvent()**

```csharp
public AdminUserUpdatedEvent()
```
